...
- AiPanel 内包含：预置功能区（错别字检查、写作建议、段落润色、生成摘要）、聊天消息区、Context 状态栏、输入框组件
- 新增 **AiInputBar** 输入框组件：底部工具栏含模型选择下拉框、模式选择（quick/think）、发送按钮
- 新增 **AiMessageBubble** 消息气泡组件：使用 markdown-it 轻量渲染 AI 回复
...
- 扩展 **appStore**：新增 aiProviders 配置（名称、端点、API Key）
- 在 **PreferencesView** 中新增 AI 提供商配置区，用户可添加多个 OpenAI 兼容端点
...
- **FrameView**：panels 注册表新增 aiPanel 条目，侧边栏新增图标按钮
- **PreferencesView**：新增 AI 提供商配置表单区
