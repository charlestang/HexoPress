...
## Purpose
分类管理模块提供分类详情页内的文章浏览与批量整理能力，包括展示直接挂载在某分类下的文章（排除子分类文章）、批量修改分类归属、批量移除分类标签，以及操作后的缓存刷新与统计更新。
...
#### Scenario: Load failure shows error in empty state
- **GIVEN** 数据加载失败
- **THEN** 系统 MUST 在空态区域展示错误信息，用户可通过刷新页面重试
...
#### Scenario: Partial failure shows summary
- **GIVEN** 批量修改中部分文章写入失败
- **WHEN** 操作完成
- **THEN** 系统 MUST 展示失败数量摘要，成功部分的修改 MUST 已生效并反映在列表中
