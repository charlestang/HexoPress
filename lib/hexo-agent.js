...
class HexoAgent {
  ...
  async createFile (directory, title, slug, content) {
    ...
    console.log('source is watching ....')
    return this.hexo.source.watch().then(async () => {
      const post = await this.hexo.post.create(data, true)
      console.log('create result is: ', post)
      if (content !== '') {
        this.saveContent(post.path.replace(this.hexo.source_dir, ''), content)
      }

      console.log('source is stop watching.')
      await this.hexo.load(() => {
        console.log('hexo load call finished.')
      })
      await this.hexo.source.unwatch()
      return post.path.replace(this.hexo.source_dir, '')
    })
  }
